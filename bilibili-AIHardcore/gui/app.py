...
class MainWindow(QMainWindow):
    ...
    def setup_settings_tab(self, tab_widget):
        ...
        self.custom_url_input.textChanged.connect(self.update_url_preview)
        custom_layout.addWidget(self.custom_url_input)

        # 添加URL预览标签
        self.url_preview_label = QLabel("最终访问URL: ")
        self.url_preview_label.setStyleSheet("color: gray; font-style: italic;")
        custom_layout.addWidget(self.url_preview_label)
    ...
    def load_settings(self):
        ...
        # 初始化URL预览
        self.update_url_preview(custom_config['base_url'])
    ...
    def update_url_preview(self, text):
        """更新URL预览标签"""
        if not text:
            self.url_preview_label.setText("最终访问URL: ")
            return

        # 根据URL构建逻辑预览最终URL
        if '/chat/completions' in text:
            final_url = text
        elif text.endswith('/v1') or text.endswith('/v1/'):
            final_url = f"{text.rstrip('/')}/chat/completions"
        else:
            final_url = f"{text.rstrip('/')}/v1/chat/completions"

        self.url_preview_label.setText(f"最终访问URL: {final_url}")
